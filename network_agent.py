...
from keras.optimizers import RMSprop  # MODIFIED: Removed .legacy
from keras.layers import Dropout  # MODIFIED: Removed .core
from keras.layers import MaxPooling2D  # MODIFIED: Removed .pooling
from keras import backend as K
import random
from keras.layers import Layer  # MODIFIED: Removed .engine.topology
import os
...
class Selector(Layer):
    ...
    def __init__(self, select, **kwargs):
        ...
        self.select = select
        # In modern Keras, constants should be handled inside call()
    ...
    def call(self, x):
        # MODIFIED: Create constant here
        select_neuron = keras.ops.convert_to_tensor(self.select, dtype=x.dtype)
        return keras.ops.cast(keras.ops.equal(x, select_neuron), dtype="float32")
...
class NetworkAgent(Agent):
    ...
    def load_model(self, file_name):
        self.q_network = load_model(os.path.join(self.path_set.PATH_TO_MODEL, "%s_q_network.h5" % file_name), custom_objects={"Selector": Selector})
    ...
    def choose(self, count, if_pretrain):
        ...
        if if_pretrain:
            ...
            if random.random() <= self.para_set.EPSILON:  # continue explore new Random Action
                ...
                self.action = np.argmax(q_values[0])
            if hasattr(self.para_set, "EPSILON_DECAY") and self.para_set.EPSILON > self.para_set.MIN_EPSILON:
                self.para_set.EPSILON *= self.para_set.EPSILON_DECAY
            elif not hasattr(self.para_set, "EPSILON_DECAY") and self.para_set.EPSILON > 0.001 and count >= 20000:
    ...
    def build_network_from_copy(self, network_copy):
        ...
        # Keras 3's RMSprop uses 'learning_rate' instead of 'lr'
